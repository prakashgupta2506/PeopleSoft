...
public class SeleniumUtil {
	...
	private static Properties configProperties;// value initialized will remain
												// same for the entire execution
												// until u use setter(creating
												// object of class and accessing
												// variable)
	private static Properties sqlProperties;
	private static final String configFile = System.getProperty("user.dir")
			+ "//src//test//resources//Config.properties";
	// private static String
	// sqlFile=System.getProperty("user.dir")+"//src//test//resources//config//SQL.properties";
	...
	/*
	 * //for one time driver object initialization public static void
	 * initializeSeleniumConnector() { SELENIUM = WebConnector.getInstance(); }
	 * 
	 * //for one time driver object initialization
	 * 
	 * public static WebDriver getDriver() { return SELENIUM.getDriver(); }
	 */

	// This code inside static block is executed only once: the first time you
	// make an object of that class or the first time you access a static member
	// of that class
	static {
		...
		try {
			...
			BROWSER_SYNC = Integer.parseInt(configProperties.getProperty("BrowserSync"));
			WEBLEMENT_SYNC = Integer.parseInt(configProperties.getProperty("WebElementSync"));
	...
	private SeleniumUtil() {// if constructor is declared as private means this
							// implements singleton concept..means object cannot
							// be created from outside...means one instance of
							// the class exist during runtime...
	...
	public WebDriver getDriver() {// one instance of webconnector implies as one
									// instance of driver....
		if (driver == null) {
			driverStatus = true;
			...
			// fireFox==null){
			if ("firefox".equalsIgnoreCase(System.getProperty("Browser"))
					|| "firefox".equalsIgnoreCase(configProperties.getProperty("BrowserName"))) {
			...
			// IE==null){
					...
					|| "IE".equalsIgnoreCase(configProperties.getProperty("BrowserName"))) {
			...
			// && chrome==null){
					...
					|| "Chrome".equalsIgnoreCase(configProperties.getProperty("BrowserName"))) {
	...
	// used for killing driver object/set driver=null, in the same tc us need to
	...
	public static void launchApplication() {
		driver.get(SeleniumUtil.getConfigProperties().getProperty("App_URL"));
	}
	...
	/*
	 * public void setFireFox(WebDriver fireFox) { this.fireFox = fireFox; }
	 * 
	 * public void setIE(WebDriver iE) { IE = iE; }
	 * 
	 * public void setChrome(WebDriver chrome) { this.chrome = chrome; }
	 */
	...
	/*
	 * public static WebElement getWebElement( By by) {
	 * LOGGER.debug("inside getWebElement"); WebElement webElement =
	 * SELENIUM.getDriver().findElement(by); return webElement; }
	 */
	...
	public static void takeScreenShot(String testMethodName) throws IOException {
		...
		FileUtils.copyFile(scrFile, new File(ScreenShotFolder+ testMethodName + strDate + ".png"));
	}

	/*
	 * public static void takeScreenShot() throws IOException { Date date = new
	 * Date(); SimpleDateFormat formatter = new
	 * SimpleDateFormat("dd-MM-yyyy-hh-mm-ss-sss"); String strDate =
	 * formatter.format(date); File scrFile = ((TakesScreenshot)
	 * SELENIUM.getDriver()).getScreenshotAs(OutputType.FILE);
	 * FileUtils.copyFile(scrFile, new File(ScreenShotFolder + "//ScreenShot" +
	 * strDate)); }
	 */
	...
	/*
	 * public static String getTitle() { return SELENIUM.getDriver().getTitle();
	 * }
	 */
	/**
	 * Close browser
	 ...
	 * SELENIUM.getDriver().manage().timeouts().implicitlyWait(BROWSER_SYNC,
	 * TimeUnit.SECONDS);
	 * 
	 * }
	 */
	...
	public static void cleanFolder( String folder) {
		LOGGER.info("delete folder contain from folder"+folder);
		File file = new File(System.getProperty("user.dir") + "//" + folder + "//");
		for (File file1 : file.listFiles()) {
				file1.delete();
			LOGGER.debug("files deleted from folder :"+folder );
		}
	...
	/*
	 * public static void ValidateWebElementPresence(By by) {
	 * LOGGER.info("Inside ValidateWebElementPresence method"); WebDriverWait
	 * explicitWait = new WebDriverWait(SELENIUM.getDriver(),WEBLEMENT_SYNC);
	 ...
	 * LOGGER.info("Inside selectWebList"); WebElement
	 * wb=SELENIUM.getDriver().findElement(by); Select sel=new Select(wb);
	 */
	public static void selectWebList(By by, String val, String selectBy) {
	...
	/*
	 * public static void selectItemWebTable(By by,int columnfirst,int
	 * columnSecond,String firstvalue,String secondvalue,String selectValue){
	 * WebElement table = SELENIUM.getDriver().findElement(by); List<WebElement>
	...
	public static void selectItemWebTable(By by, int columnfirst, int columnSecond, String firstvalue,
		...
		for (WebElement eachRow : rows) {
			...
			// =rows.get(i).findElements(By.tagName("td"));//no. of columns in
			// that particular row
