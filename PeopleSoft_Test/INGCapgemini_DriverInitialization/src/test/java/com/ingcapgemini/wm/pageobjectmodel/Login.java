...
public class Login {
	...
	public static void loginToApp(WebDriver driver,String userName ,String password){
		...
		if (userName!=null){
			SeleniumUtil.getWebElement(By.id(LOGIN_USERNAME_ID)).sendKeys(userName);
		}
		if (password!=null){
			SeleniumUtil.getWebElement(By.id(LOGIN_PASSWORD_ID)).sendKeys(password);
		}
		
		SeleniumUtil.getWebElement(By.id(LOGIN_BUTTON_ID)).click();
