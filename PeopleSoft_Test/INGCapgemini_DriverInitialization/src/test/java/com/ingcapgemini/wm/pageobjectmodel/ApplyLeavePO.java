...
public class ApplyLeavePO {
	...
	public static void applyLeave(String leaveType ,String fromDate,String toDate,String comment) throws Throwable{
		LOGGER.info("Inside ApplyLeavePO");
		
		if (leaveType!=null){
			SeleniumUtil.selectWebList(By.id(APPLYLEAVE__LEAVETYPE_ID), leaveType, "selectByVisibleText");
			Thread.sleep(1000);
		}
		if (fromDate!=null){
			SeleniumUtil.getWebElement(By.id(APPLYLEAVE__FROMDATE_ID)).clear();
			SeleniumUtil.getWebElement(By.id(APPLYLEAVE__FROMDATE_ID)).sendKeys(fromDate);
			Thread.sleep(1000);
			
		}
		if (toDate!=null){
			SeleniumUtil.getWebElement(By.id(APPLYLEAVE__TODATE_ID)).clear();
			SeleniumUtil.getWebElement(By.id(APPLYLEAVE__TODATE_ID)).sendKeys(toDate);
			Thread.sleep(1000);
			SeleniumUtil.robotSendKeys(KeyEvent.VK_TAB);
			Thread.sleep(500);
		}
		if (comment!=null){
			SeleniumUtil.getWebElement(By.id(APPLYLEAVE__COMMENT_ID)).sendKeys(comment);
		}
		...
		Thread.sleep(10000);
		SeleniumUtil.getWebElement(By.id(APPLYLEAVE__APPLYBUTTON_ID)).click();
		Thread.sleep(1000);
		SeleniumUtil.getWebElement(By.id(APPLYLEAVE__CONFIRMOKBUTTON_ID)).click();
